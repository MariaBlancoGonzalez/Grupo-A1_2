...
class WMaze:
    ...
    def __init__(self, rows, cols, option, filedata):
        self.rows = rows
        self.cols = cols
        #Classic version (asking number of rows and columns)
        if option==False:
            self.wilsonAlgorithmGen()
        #Alternative implementation taking a json file
        elif option==True:
            self.alternWilsonAlgorithm(filedata)
    ...
    def alternWilsonAlgorithm(self, data):
        ...
        for i in data['cells']:
            for j in data['cells'][i]:
                # For some reason it prints the value twice, that's why I use a counter, so that it only prints it once
                if (counter % 2 == 0):
                    WCell([i[1], i[4]]).value = data['cells'][i]['value']
                    WCell([i[1], i[4]]).neighbors = data['cells'][i]['neighbors']
                    print(WCell([i[1], i[4]]))
                    print(data['cells'][i]['neighbors'])
                    #cValue.append(data['cells'][i]['value'])  # Int type
                    #cNeigh.append(data['cells'][i]['neighbors'])  # List type
                counter += 1
